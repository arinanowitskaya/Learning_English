...
'''def quiz_view(request):
    ...
    # Выбираем 7 уникальных карточек
    ...
    if request.method == 'POST':
        ...
        # Получаем список ID карточек в том же порядке, как при генерации теста
        quiz_flashcards_ids = [card.id for card in quiz_flashcards]
        ...
        return render(request, 'flashcards/quiz_result.html', {
            'score': score,
            'total': len(quiz_flashcards),
            'questions_data': questions_data,
            'percentage': int((score / len(quiz_flashcards)) * 100) if quiz_flashcards else 0
        })
    ...
    for i, flashcard in enumerate(quiz_flashcards):
        ...
        # Выбираем случайные 3 неправильных ответа
        ...
        quiz_questions.append({
            'index': i,  # Используем индекс вместо ID
            'flashcard': flashcard,
            'options': options,
